...
//! Dashes inside directory names are ambiguous, so we resolve against the disk:
//! at each step take the longest run of segments that forms an existing dir,
//! backtracking to shorter runs when a greedy choice dead-ends.
//!
...
fn resolve(base: &Path, segs: &[&str], exists: &dyn Fn(&Path) -> bool) -> (PathBuf, bool) {
    ...
    for j in (1..=segs.len()).rev() {
        let candidate = base.join(segs[..j].join("-"));
        if !exists(&candidate) {
            continue;
        }
