...
use crate::engine::{Dimension, Severity, Source};
...
pub fn builtin_nl_rules() -> Vec<BuiltinNlRule> {
    use Dimension::{Clarity, Consistency, Examples, Structure};
    use Severity::{Hi, Lo, Mid};
    use Source::{Anthropic, Cursor, Karpathy, Openai};
    vec![
        // ── Anthropic ────────────────────────────────────────────────
        BuiltinNlRule { id: "anthropic-clarity", title: "Vague directives", severity: Mid, source: Anthropic, dimension: Clarity,
        ...
        BuiltinNlRule { id: "anthropic-examples", title: "Missing examples", severity: Mid, source: Anthropic, dimension: Examples,
            instruction: "VIOLATES if non-obvious conventions, formats, or behaviors are described without at least one concrete example." },
        BuiltinNlRule { id: "anthropic-delimit-sections", title: "Undelimited sections", severity: Lo, source: Anthropic, dimension: Structure,
        ...
        BuiltinNlRule { id: "anthropic-data-vs-instructions", title: "Data tangled into instructions", severity: Mid, source: Anthropic, dimension: Structure,
        ...
        BuiltinNlRule { id: "anthropic-allow-idk", title: "Never allows uncertainty", severity: Hi, source: Anthropic, dimension: Clarity,
        ...
        BuiltinNlRule { id: "anthropic-positive-framing", title: "Prohibitions only", severity: Lo, source: Anthropic, dimension: Clarity,
        ...
        BuiltinNlRule { id: "anthropic-context-placement", title: "Reference buried mid-instructions", severity: Lo, source: Anthropic, dimension: Structure,
        ...
        // ── OpenAI ───────────────────────────────────────────────────
        BuiltinNlRule { id: "openai-identity", title: "No agent identity", severity: Mid, source: Openai, dimension: Clarity,
            instruction: "VIOLATES if the file never states the agent's purpose, role, or the project it serves." },
        BuiltinNlRule { id: "openai-dos-and-donts", title: "No explicit dos & don'ts", severity: Mid, source: Openai, dimension: Clarity,
            instruction: "VIOLATES if behavioral rules are stated only abstractly without explicit dos and don'ts the agent can follow." },
        BuiltinNlRule { id: "openai-example-consistency", title: "Inconsistent examples", severity: Lo, source: Openai, dimension: Examples,
            instruction: "VIOLATES if examples in the file use inconsistent formatting or contradict each other." },
        BuiltinNlRule { id: "openai-structure", title: "No headers or hierarchy", severity: Lo, source: Openai, dimension: Structure,
            instruction: "VIOLATES if the file lacks any markdown headers or hierarchy and is hard to scan for distinct topics." },
        BuiltinNlRule { id: "openai-explicitness", title: "Critical requirements implied", severity: Hi, source: Openai, dimension: Clarity,
        ...
        BuiltinNlRule { id: "openai-context-early", title: "Stable context buried", severity: Lo, source: Openai, dimension: Structure,
        ...
        BuiltinNlRule { id: "openai-agentic-planning", title: "No planning guidance", severity: Lo, source: Openai, dimension: Structure,
            instruction: "VIOLATES if the file describes multi-step tasks without guiding the agent to plan or decompose before acting." },
        BuiltinNlRule { id: "openai-persistence", title: "Allows stopping early", severity: Mid, source: Openai, dimension: Consistency,
        ...
        // ── Cursor ───────────────────────────────────────────────────
        BuiltinNlRule { id: "cursor-scoped", title: "Bloated, unscoped content", severity: Mid, source: Cursor, dimension: Structure,
            instruction: "VIOLATES if the file is bloated with rambling or tangential content rather than concise, scoped instructions." },
        BuiltinNlRule { id: "cursor-specific-refs", title: "Vague file references", severity: Lo, source: Cursor, dimension: Consistency,
            instruction: "VIOLATES if it points to \"the relevant files\" or \"the config\" vaguely instead of naming specific files or paths." },
        BuiltinNlRule { id: "cursor-declare-conventions", title: "Stack & commands undeclared", severity: Mid, source: Cursor, dimension: Consistency,
            instruction: "VIOLATES if the project's stack, build/test commands, or core conventions are never declared." },
        BuiltinNlRule { id: "cursor-one-concern", title: "Sections mix concerns", severity: Lo, source: Cursor, dimension: Structure,
            instruction: "VIOLATES if a single section tries to govern many unrelated concerns that should be split." },
        BuiltinNlRule { id: "cursor-no-stale-blanket", title: "Stale blanket guidance", severity: Lo, source: Cursor, dimension: Consistency,
        ...
        BuiltinNlRule { id: "cursor-code-style-examples", title: "Style without example", severity: Lo, source: Cursor, dimension: Examples,
            instruction: "VIOLATES if it mandates a code style without showing a concrete example of compliant code." },
        // ── Karpathy / community ─────────────────────────────────────
        BuiltinNlRule { id: "community-success-criteria", title: "No definition of done", severity: Mid, source: Karpathy, dimension: Clarity,
            instruction: "VIOLATES if tasks are described without any definition of done or success criteria." },
        BuiltinNlRule { id: "community-no-dead-context", title: "Dead reference content", severity: Lo, source: Karpathy, dimension: Consistency,
            instruction: "VIOLATES if it includes reference content that is never connected to any instruction or used by any task." },
        BuiltinNlRule { id: "community-single-source", title: "Scattered duplicate guidance", severity: Mid, source: Karpathy, dimension: Consistency,
        ...
        BuiltinNlRule { id: "community-concrete-over-abstract", title: "Abstract where concrete needed", severity: Lo, source: Karpathy, dimension: Clarity,
