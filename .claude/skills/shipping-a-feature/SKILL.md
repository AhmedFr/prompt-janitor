...
4. **Local gates** — `pnpm check` must pass before the PR opens (frontend lint/test/build/storybook, `cargo fmt/clippy/test` in `src-tauri`, fulfillment typecheck/test, landing build). This IS the merge gate: GitHub Actions runs the same suite only on pushes to `main` (the Rust job needs a macOS runner billed at 10x), so no check ever runs on a PR. `pnpm install` wires `pnpm check:fast` into a pre-push hook; `SKIP_CHECKS=1 git push` bypasses it — only for emergencies.
